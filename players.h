...
extern bool CalcScreen(const float *origin, float *vecScreen);
extern void DoExtraRendering(cl_entity_s * ent, int team);
extern ColorEntry * PlayerColor(int ax);
extern void DrawEntityEsp(void);
extern void drawPlayerEsp(int ax);
extern bool bIsEntValid(cl_entity_s * ent,int index);
extern float mainViewAngles[3];
extern void playerCalcExtraData(int ax, cl_entity_s* ent);
extern bool bDoAimKey(int eventcode);
